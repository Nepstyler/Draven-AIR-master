        ...
        public RemotingMessageReceivedEventArgs HandleMessage(object sender, RemotingMessageReceivedEventArgs e)
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connString))
                {
                    ...
                    {
                        SpellBookPageDTO p1 = new SpellBookPageDTO { PageId = 1, Name = "Rune Page 1", Current = true, SummonerId = summonerId, SlotEntries = new ArrayCollection() };
                        SpellBookPageDTO p2 = new SpellBookPageDTO { PageId = 2, Name = "Rune Page 2", Current = false, SummonerId = summonerId, SlotEntries = new ArrayCollection() };
                        book.BookPages.Add(p1);
                        book.BookPages.Add(p2);
                    }
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine("[ERROR] Eroare DB GetSpellBook: " + ex.Message);
            }
